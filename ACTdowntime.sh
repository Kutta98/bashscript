...
check_packet_loss() {
    ...
    if [ -n "$packet_loss" ] && [ "$packet_loss" -gt "$threshold" ]; then
        ...
        echo " Office ACT ISP is currently experiencing a status downgrade $ip_address: $packet_loss%"| mail -s "ACT ISP STATUS NOTIFICATIONS" gokulnath.b@golden-hippo.com rajesh.r@golden-hippo.com
       ...
       # echo "Packet loss within acceptable limits for $ip_address: $packet_loss%"| mail -s "ACT ISP STATUS NOTIFICATION" gokulnath.b@golden-hippo.com
    fi
}

# Specify the IP address to ping
ip_to_ping="123.176.34.1"  # Replace with your desired IP address
